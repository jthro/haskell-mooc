...
firstHalf s = take ((length s + length s `mod` 2) `div` 2) s
...
joinToLength n xs = filter (\x -> length x == n) [x ++ y | x <- xs, y <- xs]
...
interpreter commands = interpreter' commands 0 0
  where
    ...
    interpreter' [] x y = []
    interpreter' (s:ss) x y
      | s == "printX" = show x : interpreter' ss x y
      | s == "printY" = show y : interpreter' ss x y
      | s == "up" = interpreter' ss x (y + 1)
      | s == "down" = interpreter' ss x (y - 1)
      | s == "left" = interpreter' ss (x - 1) y
      | s == "right" = interpreter' ss (x + 1) y
